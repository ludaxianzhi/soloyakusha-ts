...
describe("computeChunkLinkGraph", () => {
  test("builds bidirectional cross-block pair strengths with sqlite-memory vectors", async () => {
    ...
    try {
      ...
      // Each directed block pair aggregates 2 bidirectional chunk pairs, but each
      // direction only accumulates its own cosine square while still requiring the
      // reverse strong edge to exist. Total ≈ 2 * (0.9801 / 0.9802) ≈ 1.9998.
      expect(result.blockPairStrengths).toBeInstanceOf(Float32Array);
      expect(result.blockPairStrengths[0]).toBeCloseTo(2, 0);
      expect(result.blockPairStrengths[1]).toBeCloseTo(2, 0);
