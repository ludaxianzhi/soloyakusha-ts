...
- The TUI (`src\tui\`) is a thin Ink shell around shared providers and screens.
...
- `ProjectService` in `src\webui\services\project-service.ts` is the main orchestration layer for the WebUI. It owns the active `TranslationProject`, emits snapshots/logs/progress to the SSE bus, and reuses the same project/domain modules as the TUI instead of duplicating business logic in the UI layer.

## Key conventions

- Tests are colocated with implementation files and use the `*.test.ts` naming pattern with `bun:test`.
- Prefer extending shared domain/services instead of adding UI-specific business logic. TUI and WebUI are both wrappers over the same `src\project`, `src\config`, `src\llm`, `src\glossary`, and file-handler modules.
